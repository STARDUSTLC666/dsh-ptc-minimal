// dsh-ptc-minimal 宿主行：把内置的 ptc-minimal 预设物化到用户预设根目录。
//
...
function materialize(logger) {
  ...
  if (marker === null && existsSync(join(dir, 'agent.cordis.yml'))) {
    logger?.warn(`dsh-ptc-minimal: ${dir} exists without our marker; leaving it alone`)
    return
  }
  ...
  logger?.info(`dsh-ptc-minimal: materialized agent preset "${PRESET_ID}" into ${dir}`)
}
...
export function apply(ctx) {
  try {
    ...
    ctx.logger?.warn(`dsh-ptc-minimal: preset materialization failed: ${String(error)}`)
  }
}
